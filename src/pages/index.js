...
const App = () => {
  ...
  const handleLanguageSelect = (language) => {
    router.push(`/quiz?language=${language.trim().toLowerCase()}&user_id=${userId}`);
  };
    ...
    <Layout animation={animation}>
      <div className="relative h-full w-full z-50 overflow-hidden">
        ...
        <div className="px-12 sm:w-4/5 sm:mx-auto">
            ...
            >
              ...
              <div className="w-full flex flex-col gap-1 justify-center">
                <input
                  ...
                  value={Name}
                  onChange={(e) => setName(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && Name.trim() && goForward()}
                />
              </div>
          ...
          {showLanguage && (
            <section
              className={`flex w-full flex-col gap-6 mt-6 transition-all duration-700 ease-in-out ${showLanguage ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"
                }`}
            >
              <h1 className="font-bold text-xl/7 text-[#4B2500] text-center">CHOOSE LANGUAGE</h1>
